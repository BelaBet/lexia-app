  ...
  if(selectedReport){
    ...
    return <div className="mx-auto w-full max-w-[1220px] space-y-8 pb-8 sm:space-y-10">
      ...
      <section className="pt-3">
        ...
        <div className="divide-y rounded-lg border md:hidden">{pagedResults.map((row)=><article key={row.id} className="p-4"><button onClick={()=>row.case_id&&navigate(`/processos/${row.case_id}`)} className="break-all text-left font-mono text-xs font-semibold text-primary">{row.process_number||"—"}</button><p className="mt-2 text-xs leading-relaxed">{partyLabel(row)}</p><div className="mt-3 grid grid-cols-2 gap-3 text-xs"><div><span className="text-muted-foreground">Distribuição</span><p>{fmtDate(row.data_distribuicao)}</p></div><div><span className="text-muted-foreground">Tribunal</span><p>{row.tribunal||"—"}</p></div><div><span className="text-muted-foreground">Valor</span><p>{row.valor!=null?currency.format(Number(row.valor)):"—"}</p></div><div><span className="text-muted-foreground">Área</span><p>{row.area||"—"}</p></div><div className="col-span-2"><span className="text-muted-foreground">Natureza</span><p>{row.natureza||safeText(raw(row).classeNatureza)||"—"}</p></div></div><div className="mt-4 flex gap-2"><Button variant="outline" size="sm" onClick={()=>downloadCsv(`${row.process_number||"processo"}.csv`,[row])}><Download className="h-4 w-4"/></Button><Button size="sm" className="flex-1" disabled={!row.case_id} onClick={()=>row.case_id&&navigate(`/processos/${row.case_id}`)}>Ver detalhes</Button></div></article>)}</div>

        <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[1040px] text-sm"><thead className="border-y text-left text-xs text-muted-foreground"><tr><th className="px-3 py-3 font-medium">Número do processo</th><th className="px-3 py-3 font-medium">Parte</th><th className="px-3 py-3 font-medium">Distribuição</th><th className="px-3 py-3 font-medium">Tribunal</th><th className="px-3 py-3 font-medium">Valor (R$)</th><th className="px-3 py-3 font-medium">Área</th><th className="px-3 py-3 font-medium">Natureza</th><th className="px-3 py-3 font-medium">Ações</th></tr></thead><tbody>{pagedResults.map((row)=><tr key={row.id} className="border-b align-top hover:bg-muted/20"><td className="px-3 py-4"><button className="font-mono text-xs text-primary hover:underline" onClick={()=>row.case_id&&navigate(`/processos/${row.case_id}`)}>{row.process_number||"—"}</button></td><td className="max-w-[330px] px-3 py-4 text-xs leading-5">{partyLabel(row)}</td><td className="px-3 py-4">{fmtDate(row.data_distribuicao)}</td><td className="px-3 py-4">{row.tribunal||"—"}</td><td className="px-3 py-4">{row.valor!=null?currency.format(Number(row.valor)):"—"}</td><td className="px-3 py-4">{row.area||"—"}</td><td className="max-w-[220px] px-3 py-4 text-xs leading-5">{row.natureza||safeText(raw(row).classeNatureza)||"—"}</td><td className="px-3 py-4"><div className="flex gap-2"><Button variant="outline" size="sm" onClick={()=>downloadCsv(`${row.process_number||"processo"}.csv`,[row])}><Download className="h-4 w-4"/></Button><Button size="sm" disabled={!row.case_id} onClick={()=>row.case_id&&navigate(`/processos/${row.case_id}`)}>Abrir</Button></div></td></tr>)}</tbody></table></div>
