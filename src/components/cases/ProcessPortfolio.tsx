...
export function ProcessPortfolio(){
  ...
  // Agregações dos gráficos (ano, natureza, status, polo, estado) só dependem
  // de selectedResults — memoizadas para não recalcular tudo a cada
  // keystroke na busca ou troca de página, que só afetam filteredResults.
  const chartData=useMemo(()=>{
    if(!selectedReport) return null;
    return {
      expectedTotal:getExpectedTotal(selectedReport),
      latestMovement:latestDate(selectedResults),
      years:yearData(selectedResults),
      natures:natureData(selectedResults),
      statuses:statusData(selectedResults),
      polos:poloData(selectedResults),
      states:stateData(selectedResults),
    };
  },[selectedReport,selectedResults]);
  const normalizeSearch=(value:unknown)=>String(value??"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]/g,"");
  const filteredResults=useMemo(()=>{let rows=selectedResults;if(chartFilter){rows=rows.filter((row)=>{const source=raw(row);if(chartFilter.kind==="state")return safeText(source.uf).toUpperCase()===chartFilter.value;if(chartFilter.kind==="year")return (row.data_distribuicao||"").includes(chartFilter.value);if(chartFilter.kind==="nature")return (row.area?.trim()||safeText(source.classeNatureza)||row.natureza?.trim()||"OUTROS")===chartFilter.value;if(chartFilter.kind==="polo"){const papel=safeText(source.papel);const value=/ativo/i.test(papel)?"Polo ativo":/passivo|réu|reu/i.test(papel)?"Polo passivo":papel||"Sem dados";return value===chartFilter.value;}const status=(row.status_processual||"").toLowerCase();const closed=Boolean(source.arquivado)||Boolean(source.extinto)||/arquiv|baixad|encerr|extint/.test(status);return (closed?"Encerrado":"Não encerrado")===chartFilter.value;});}const term=searchTerm.trim().toLowerCase();if(!term)return rows;const normalizedTerm=normalizeSearch(term);return rows.filter((row)=>{const searchable=[row.tribunal,row.process_number,row.area,row.natureza,row.comarca,row.vara,row.status_processual,partyLabel(row)].filter(Boolean).join(" ");const normalizedSearchable=normalizeSearch(searchable);return searchable.toLowerCase().includes(term)||(normalizedTerm.length>0&&normalizedSearchable.includes(normalizedTerm));});},[selectedResults,searchTerm,chartFilter]);
